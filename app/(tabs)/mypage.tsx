...
export default function MyPageScreen() {
  ...
  const quickMenus = [
    { label: "AI 매칭 내역", icon: TrendingUp, action: () => router.push('/lost-item') }, // Link to appropriate screens
    { label: "주은 물품 목록", icon: Package, action: () => router.push('/lost-item') },
    { label: "채팅 목록", icon: MessageCircle, action: () => router.push('/chat') },
    { label: "스캔 내역", icon: QrCode, action: () => router.push('/scan') },
  ];
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View className="bg-white px-4 py-6 border-b border-gray-100">
          <View className="flex-row items-center">
            ...
            <View className="flex-1 ml-5">
              <Text className="text-xl font-pretendard-bold text-gray-900">{profile?.nickname ?? '사용자'}</Text>
        ...
        <View className="flex-row bg-white border-b border-gray-100">
          {[
            ...
            <View key={stat.label} className={`flex-1 items-center py-4 ${idx < 2 ? 'border-r border-gray-50' : ''}`}>

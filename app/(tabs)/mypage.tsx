...
export default function MyPageScreen() {
    ...
    const quickMenus = [
        {label: "AI 매칭 내역", icon: TrendingUp, action: () => router.push(ROUTES.LOST_ITEM_BOARD)}, // Link to appropriate screens
        {label: "주은 물품 목록", icon: Package, action: () => router.push(ROUTES.LOST_ITEM_BOARD)},
        {label: "채팅 목록", icon: MessageCircle, action: () => router.push(ROUTES.CHAT)},
        {label: "스캔 내역", icon: QrCode, action: () => router.push(ROUTES.SCAN)},
    ];
    ...
    return (
        <View className="flex-1 bg-gray-50" style={{paddingTop: insets.top}}>
            ...
            <View className="px-4 py-4 flex-row items-center justify-between bg-white border-b border-gray-100">
                <Text className="text-xl font-pretendard-bold text-gray-900">내 정보</Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View className="bg-white px-4 py-6 border-b border-gray-100">
                    <View className="flex-row items-center">
                        <View className="relative">
                            <View
                                className="w-20 h-20 rounded-full bg-indigo-50 items-center justify-center border-2 border-indigo-100 overflow-hidden">
                        ...
                        <View className="flex-1 ml-5">
                            <Text
                                className="text-xl font-pretendard-bold text-gray-900">{profile?.nickname ?? '사용자'}</Text>
                            <Text className="text-sm font-pretendard-medium text-indigo-600 mt-1">
                                {profile?.department ?? '학과 정보 없음'}
                            </Text>
                            <Text className="text-xs font-pretendard-regular text-gray-400 mt-1">명지대학교 자연캠퍼스</Text>
                        </View>
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => setIsEditModalVisible(true)}
                        >
                            <Text className="text-xs font-pretendard-semibold text-indigo-600 mr-1">프로필 수정</Text>
                ...
                <View className="flex-row bg-white border-b border-gray-100">
                    {[
                        ...
                        <View key={stat.label}
                            ...
                            <Text className="text-lg font-pretendard-bold text-gray-900">{stat.value}</Text>
                            <Text className="text-xs font-pretendard-medium text-gray-400 mt-1">{stat.label}</Text>
                        </View>
                    ))}
                </View>
                ...
                <View className="mt-4 mx-4 bg-white rounded-3xl p-5 border border-gray-100 shadow-sm">
                    <Text className="text-sm font-pretendard-bold text-gray-800 mb-4">활동 내역</Text>
                    <View className="flex-row flex-wrap justify-between">
                        {quickMenus.map((menu, idx) => (
                            ...
                            >
                                <View
                                    className="w-8 h-8 rounded-full bg-white items-center justify-center mr-3 shadow-sm">
                                    <menu.icon size={16} color="#4F6EF7"/>
                                </View>
                                <Text className="text-xs font-pretendard-semibold text-gray-700">{menu.label}</Text>
                            </TouchableOpacity>
                        ))}
                    </View>
                </View>
                ...
                <View className="mt-4 mx-4 bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm">
                    <Text className="text-sm font-pretendard-bold text-gray-800 px-5 pt-5 pb-3">설정</Text>
                    ...
                    <View className="flex-row items-center justify-between px-5 py-4 border-t border-gray-50">
                        <View className="flex-row items-center">
                            ...
                            <Text className="text-sm font-pretendard-medium text-gray-700">푸시 알림 설정</Text>
                        </View>
                        <Switch
                            value={pushEnabled}
                    ...
                    <View className="flex-row items-center justify-between px-5 py-4 border-t border-gray-50">
                        <View className="flex-row items-center">
                            ...
                            <Text className="text-sm font-pretendard-medium text-gray-700">매칭 채팅만 받기</Text>
                        </View>
                        <Switch
                            value={chatOnlyMode}
                    ...
                    <TouchableOpacity
                        ...
                        <View className="flex-row items-center">
                            ...
                            <Text className="text-sm font-pretendard-medium text-gray-700">개인정보 처리방침</Text>
                        </View>
                ...
                <View className="mt-4 mx-4 mb-10 bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm">
                    <Text className="text-sm font-pretendard-bold text-gray-800 px-5 pt-5 pb-3">계정</Text>
                    ...
                    <TouchableOpacity
                        ...
                        <View className="flex-row items-center">
                            ...
                            <Text className="text-sm font-pretendard-medium text-gray-700">비밀번호 변경</Text>
                        </View>
                        <ChevronRight size={18} color="#D1D5DB"/>
                    </TouchableOpacity>
                    ...
                    >
                        <View className="flex-row items-center">
                            ...
                            <Text className="text-sm font-pretendard-medium text-red-500">로그아웃</Text>
                        </View>

...
import { UpdateProfileRequest, UserProfile } from '@/api/types';
import { DEPARTMENTS } from '@/constants/departments';
import { useMutation, useQueryClient } from '@tanstack/react-query';
import { Check, ChevronDown, ChevronUp, Info, Search, X } from 'lucide-react-native';
                    ...
                    <ScrollView className="flex-1 px-6" showsVerticalScrollIndicator={false}>
                        ...
                        <View className="items-center py-8">
                            <View className="relative">
                                <View
                                    className="w-24 h-24 rounded-full bg-indigo-50 items-center justify-center border-2 border-indigo-100 overflow-hidden">
                        ...
                        <View className="mb-6">
                            ...
                            >
                                ...
                                {showDeptDropdown ? <ChevronUp size={20} color="#9CA3AF"/> :
                                    <ChevronDown size={20} color="#9CA3AF"/>}
                            </TouchableOpacity>

                            {showDeptDropdown && (
                                <View
                                    className="mt-2 bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm max-h-64">
                                    <View
                                        className="flex-row items-center bg-gray-50 border-b border-gray-50 px-4 py-2">
                                    ...
                                    <ScrollView nestedScrollEnabled={true}>
                                        ...
                                        {filteredDepts.length === 0 && (
                                            <View className="py-8 items-center">
                                                <Text className="text-sm font-pretendard-medium text-gray-400">검색 결과가
                                                    없습니다.</Text>
                                            </View>
                                        )}
                                    </ScrollView>
